...
class Manifold extends React.Component {
	...
	initialize_indicator_timeline() {
		let currentNeedleRotation = gsap.getProperty(this.pressureNeedle, "rotation");
		...
		this.tl_pressNeedleMove = new TimelineMax({paused: true, onComplete: () => {
			this.end_tl();
		}});
	...
	initialize_fueldump_timeline() {
		this.tl_fuelSpray = new TimelineMax();
	...
	componentDidMount() {
		if (this.props.id === 'crossfeed_line2') {
			TweenMax.set(this.pressureNeedle, {transformOrigin: '18.35% 25.02%'});
		}
	}
...
class RotaryValve extends React.Component {
	...
	componentDidMount() {
		TweenMax.set(this.el, {transformOrigin: 'center'});
...
class SpringValve extends React.Component {
	...
	componentDidMount() {
		...
		TweenMax.set(this.el, {transformOrigin: transform_origin});
...
class FuelPump extends React.Component {
	constructor(props) {
		...
		this.tl= new TimelineMax({paused: true});
		this.light_tl = new TimelineMax({paused: true});
...
class RotarySwitch extends React.Component {
	...
	componentDidMount() {
		...
		if (this.props.id.indexOf('bypass') > -1) {
			TweenMax.set(this.switchGroup, {transformOrigin: 'center', rotate: 90});
		}
...
class QuantityIndicator extends React.Component {
	constructor(props) {
		...
		this.rotDuration = 2;
		this.tl_indicatorNeedleMove= new TimelineMax({ paused: false, onComplete: () => {
			this.end_tl();
			} 
		});
		this.tl_indicatorTest= new TimelineMax({ paused: true, onComplete: () => {
	...
	componentDidMount() {
		...
		TweenMax.set(this.indicatorNeedle, {transformOrigin: '77% 17.6%', rotate: this.props.rotationDeg});
		TweenMax.set(this.indicatorTestButton, {transformOrigin: '50% 50%'});

		this.tl_indicatorTest.to(this.indicatorNeedle, {rotate: 0, duration: 3});
	}

	componentDidUpdate(prevProps) {
		let currentNeedleRotation = gsap.getProperty(this.el, "rotation");
		...
		if (this.props.dumping === 'stop' || this.props.dumping === 'has stopped') {
			...
			console.log('stop rot: ', gsap.getProperty(this.indicatorNeedle, "rotation"));
		}

		if (this.props.dumping === 'stop') {
			this.props.updateQuantity(this.props.id, gsap.getProperty(this.indicatorNeedle, "rotation"));
		}
	}
	...
	*/
	...
	componentDidUpdate (prevProps, prevState) {
		...
		if ((this.state.stateTestVal == 0) || (prevState.stateTestVal !== this.state.stateTestVal)) {
				...
				newManifolds.map((manifold) => {
					...
					if (manifold.fuelTrap) {
						let trapValves = newRotaryValves.filter((valve) => {
							if (manifold.fuelTrap.toString().match(valve.id)) {
								return valve;
							}
						});
			...
			------------------------- */
				newRotaryValves.map((valve) => {
					let initfuelstate = valve.fuelPresent;
					let fuelpresent = false;
					valve.fuelSources.map((fuelsource)  => {
